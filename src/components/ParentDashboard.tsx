  ...
  const stats = [
    {
      ...
      value: user?.totalSponsorships ? `${user.totalSponsorships}` : "0",
      icon: Users,
      color: "from-purple-500 to-pink-500"
    },
    {
      label: "Total Contributions",
      value: user?.totalSponsorshipAmount ? `${user.totalSponsorshipAmount} ESP` : "0 ESP",
      icon: BookOpen,
      color: "from-blue-500 to-cyan-500"
    },
    {
      label: "Last Contribution",
      value: user?.lastSponsorshipDate ? new Date(user.lastSponsorshipDate).toLocaleDateString() : "N/A",
      icon: Calendar,
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
            {sponsorshipTiers.map((tier, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className={`${tier.color} rounded-3xl p-8 border-4 ${tier.highlighted ? "border-white" : "border-white/50"
              ...
              >
                <div className="text-center">
                  ...
                  <button
                    ...
                    className={`w-full py-3 px-6 rounded-full transition-all text-center ${tier.highlighted
